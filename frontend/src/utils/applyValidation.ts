...
export function validateApplicationForm(input: {
  name: string;
  email: string;
  resumeUrl: string;
  coverNote: string;
  ...
  return (
    validateApplicantName(input.name) ??
    validateApplicantEmail(input.email) ??
    validateResumeUrl(input.resumeUrl) ??
    validateCoverNote(input.coverNote)
  );
}
